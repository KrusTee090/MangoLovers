/* ═══════════════════════════════════════════════════════════
   ...
   async function loadProducts() {
     try {
       ...
       // category_id is a FK to categories — resolve to name via dbCategories
       const mapped = (data || []).map(row => {
         const catObj = dbCategories.find(c => c.id === row.category_id);
         return {
           id:          row.id,
           name:        row.name,
           category:    catObj ? catObj.name : 'Uncategorized',
   ...
   async function saveProduct(formData) {
     try {
       ...
       const catObj = dbCategories.find(c => c.name === formData.category);
       const payload = {
         name:           formData.name,
   ...
   // Global categories array — populated from DB
   let dbCategories = [];
   ...
   async function loadCategories() {
     try {
       ...
       if (error) throw error;
       dbCategories = data || [];
       // Refresh all category dropdowns and the categories table
